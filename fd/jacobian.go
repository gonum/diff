...
// the location x. The Jacobian matrix J is the matrix of all first-order
// partial derivatives of f. If f maps n-dimensional vectors x to m-dimensional
// vectors y = f(x), J is an m×n matrix given as
//      ⎡ ∂f_1 / ∂x_1 ... ∂f_1 / ∂x_n ⎤
//      ⎢      .   .           .      ⎥
//  J = ⎢      .       .       .      ⎥
//      ⎢      .           .   .      ⎥
	...
	for j := 0; j < c; j++ {
		...
		for _, pt := range formula.Stencil {
			if pt.Loc == 0 {
				...
				xcopy[j] += pt.Loc * step
				f(y, xcopy)
				col.AddScaledVec(col, pt.Coeff, yVec)
			}
		}
	}
